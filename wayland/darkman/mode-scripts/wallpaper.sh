...
if pgrep niri; then
	...
	outputs=$(swww query)
	[[ "$outputs" == *"DP-1"* ]] && swww img -o DP-1 "${LANDSCAPE_IMG}" --transition-type wipe --transition-fps 165 --transition-step 10 --transition-angle 250
	[[ "$outputs" == *"DP-3"* ]] && swww img -o DP-3 "${PORTRAIT_IMG}" --transition-type wipe --transition-fps 75 --transition-step 10 --transition-angle 250
